...
def client_liste_envies_show():
    mycursor = get_db().cursor()
    id_client = session['id_user']
    equipements_liste_envies = []
    equipements_historique = []
    return render_template('client/liste_envies/liste_envies_show.html'
                           ,equipements_liste_envies=equipements_liste_envies
                           , equipements_historique=equipements_historique
                           #, nb_liste_envies= nb_liste_envies
                           )



def client_historique_add(equipement_id, client_id):
    mycursor = get_db().cursor()
    client_id = session['id_user']
    # rechercher si l'equipement pour cet utilisateur est dans l'historique
    # si oui mettre
    sql ='''   '''
    mycursor.execute(sql, (equipement_id, client_id))
...
def client_liste_envies_equipement_move():
    mycursor = get_db().cursor()
    id_client = session['id_user']
    id_equipement = request.args.get('id_equipement')
